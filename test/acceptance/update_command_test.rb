...
class UpdateCommandTest < Minitest::Test
  ...
  # Baseline test: verify create command works correctly
  ...
  def test_update_with_scale_down
    ...
    refute container.file_exists?("/etc/systemd/system/myapp-web.2.service"),
  ...
  def test_update_with_new_process_type
    ...
    refute container.file_exists?("/etc/systemd/system/myapp-worker.1.service")
    ...
    assert container.file_exists?("/etc/systemd/system/myapp-worker.1.service"),
  ...
  def test_update_with_exec_start_change
    ...
    # Get the initial PID
    pid_result = container.exec_as_root("systemctl show myapp-web.1.service --property=MainPID --value")
